...
var config = {
    ...
    scripts: 'app/**/*.js',
    cryptoScripts:'app/crypto/external/*.js',
    forks:'app/forks/**/*.js',
...
gulp.task('watch:scripts', function () {
    gulp.src([config.scripts,'!'+config.cryptoScripts,'!'+config.forks])
...
gulp.task('copy:electron', function() {
    return gulp.src([config.app + '/main.js',config.app + '/package.json'])
               .pipe(gulp.dest(config.dist ));
});

gulp.task('build', ['clean:dist'], function () {
    plugins.runSequence(['copy:html','copy:fonts','copy:images','copy:electron', 'index:build']);
});
